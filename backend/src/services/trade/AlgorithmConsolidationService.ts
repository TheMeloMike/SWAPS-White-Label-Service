  ...
  private async executeCanonicalDiscovery(
    ...
    try {
      ...
      const canonicalConfig: AdvancedCycleEngineConfig = {
        ...
        // Advanced optimizations (MVP OPTIMIZED - Earlier Algorithm Activation)
        enableLouvainClustering: wallets.size > 7,  // MVP: Enable clustering earlier (was 10)
        enableBloomFilters: wallets.size > 15,      // MVP: Enable filters earlier (was 20) 
        enableKafkaDistribution: wallets.size > 50, // Keep large scale threshold
        enableParallelProcessing: wallets.size > 3, // MVP: Enable parallelization earlier (was 5)
        maxCommunitySize: 50,
        bloomFilterCapacity: Math.max(2000, wallets.size * 150), // MVP: +50% larger capacity, higher base
        kafkaBatchSize: 10,
        parallelWorkers: Math.min(6, Math.max(1, Math.floor(wallets.size / 8))) // MVP: More workers, lower ratio
      };
