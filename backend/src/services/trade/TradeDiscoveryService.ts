...
export class TradeDiscoveryService implements ITradeDiscoveryService {
  ...
  /**
   * CRITICAL PRODUCTION FIX - Phase 1.2
   * 
   * Update all data structures from external source (e.g., PersistentTradeDiscoveryService)
   * This method enables the live graph architecture to sync data to the algorithm layer
   */
  public async updateDataStructures(
    ...
    const operation = this.logger.operation('updateDataStructures');
    operation.info('Updating data structures from external source', {
      walletsCount: wallets.size,
      nftsCount: nftOwnership.size,
      wantsCount: wantedNfts.size,
      rejectionsCount: rejectionPreferences?.size || 0
    });

    try {
      // Use mutex locks to ensure thread safety during bulk updates
      await this.walletsMutex.runExclusive(async () => {
        this.wallets.clear();
        for (const [walletId, walletState] of wallets) {
          this.wallets.set(walletId, walletState);
        }
      });

      await this.nftOwnershipMutex.runExclusive(async () => {
        this.nftOwnership.clear();
        for (const [nftId, ownerId] of nftOwnership) {
          this.nftOwnership.set(nftId, ownerId);
        }
      });

      await this.wantedNftsMutex.runExclusive(async () => {
        this.wantedNfts.clear();
        for (const [walletId, wantSet] of wantedNfts) {
          this.wantedNfts.set(walletId, new Set(wantSet));
        }
      });

      if (rejectionPreferences) {
        await this.rejectionPreferencesMutex.runExclusive(async () => {
          this.rejectionPreferences.clear();
          for (const [walletId, prefs] of rejectionPreferences) {
            this.rejectionPreferences.set(walletId, prefs);
          }
        });
      }

      operation.info('Data structures updated successfully', {
        walletsUpdated: this.wallets.size,
        nftsUpdated: this.nftOwnership.size,
        wantsUpdated: this.wantedNfts.size,
        rejectionsUpdated: this.rejectionPreferences.size
      });

    } catch (error) {
      operation.error('Failed to update data structures', { error: (error as Error).message });
      throw error;
    }
  }

  /**
   * Get current data structure stats for diagnostics
   */
  public getDataStats(): {
    walletCount: number;
    nftCount: number;
    ...
    return {
      ...
      nftCount: this.nftOwnership.size,
      wantCount: this.wantedNfts.size,
      rejectionCount: this.rejectionPreferences.size,
      lastSync: new Date()
    };
  }

  /**
   * Clear all data structures - for testing tenant isolation
   */
  public async clearAllData(): Promise<void> {
    const operation = this.logger.operation('clearAllData');
    
    try {
      await Promise.all([
        this.walletsMutex.runExclusive(async () => this.wallets.clear()),
        this.nftOwnershipMutex.runExclusive(async () => this.nftOwnership.clear()),
        this.wantedNftsMutex.runExclusive(async () => this.wantedNfts.clear()),
        this.rejectionPreferencesMutex.runExclusive(async () => this.rejectionPreferences.clear())
      ]);

      operation.info('All data structures cleared successfully');
    } catch (error) {
      operation.error('Failed to clear data structures', { error: (error as Error).message });
      throw error;
    }
  }
