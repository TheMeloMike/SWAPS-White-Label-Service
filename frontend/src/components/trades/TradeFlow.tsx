...
export const TradeFlow = ({ steps, efficiency, id, onReject }: TradeFlowProps): JSX.Element => {
  ...
  const handleExecuteTrade = async () => {
    ...
    try {
      ...
      // The smart contract service expects a string trade ID that it will convert to 32 bytes
      // We'll use the existing ID string directly
      const tradeId = id.padEnd(32, '0').substring(0, 32); // Ensure it's exactly 32 characters
      
      const [tradeLoopAddress] = PublicKey.findProgramAddressSync(
        [Buffer.from('trade_loop'), Buffer.from(tradeId)],
                          ...
                          {step.nfts.map((nft) => (
                            <div key={nft.address}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', marginTop: '2px' }}>
                                ...
                                {nft.isMagicEdenBadged && (
                                  <span style={{ fontSize: '0.6rem', color: '#00E0B5', fontWeight: 'bold' }} title="Verified by Magic Eden">
                                    (ME ✓)
                                  </span>
                                )}
                              </div>
