...
export class NFTService extends BaseService {
  ...
  async fetchNFTsByAddresses(addresses: string[]): Promise<Map<string, NFTMetadata>> {
    try {
      ...
      const data = await this.apiPost<{ nfts: NFTMetadata[] }>(
        '/api/nfts/batch', 
        { mintAddresses: uncachedAddresses }
      );
  ...
  /**
   * Fetches floor prices for a list of NFTs
   * @param nfts - Array of NFTs to fetch floor prices for
   * @returns Promise<void> - Updates the NFTs in-place with floor prices
   */
  async getFloorPrices(nfts: NFTMetadata[]): Promise<void> {
    try {
      if (!nfts || nfts.length === 0) return;
      
      // Extract collection IDs or mint addresses for the API request
      const nftIdentifiers = nfts.map(nft => {
        // Prefer collection ID if available
        if (typeof nft.collection === 'string' && nft.collection.length > 0) {
          return nft.collection;
        } else if (typeof nft.collection === 'object' && nft.collection?.address) {
          return nft.collection.address;
        }
        // Fall back to NFT address
        return nft.address;
      });
      
      console.log(`Fetching floor prices for ${nftIdentifiers.length} NFTs`);
      
      // Check if endpoint exists
      const endpointExists = await this.checkApiEndpointExists('/api/nfts/floor-prices');
      ...
      if (endpointExists) {
        try {
          ...
          }>('/api/nfts/floor-prices', { nftIdentifiers });
          
          if (response.success && response.floorPrices) {
            // Update the NFTs with floor prices
            nfts.forEach(nft => {
              // Try to match using collection ID first
              let collectionId = '';
              ...
              // If we found a floor price for this collection, apply it
              if (collectionId && response.floorPrices[collectionId]) {
                nft.floorPrice = response.floorPrices[collectionId];
              } else if (response.floorPrices[nft.address]) {
                // Try using the NFT's own address as a fallback
                nft.floorPrice = response.floorPrices[nft.address];
              }
            });
          }
        } catch (error) {
          console.warn('Error fetching floor prices from API:', error);
        }
      } else {
        console.log('Floor prices API endpoint not available');
      }
    } catch (error) {
      console.error('Error in getFloorPrices:', error);
    }
  }
} 